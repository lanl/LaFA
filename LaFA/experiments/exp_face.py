...
def read_pgm(pgmf):
    ...
    header1 = pgmf.readline()
    header2 = pgmf.readline()
    assert header[:2] == b'P5'
    (width, height) = [int(i) for i in header1.split()[:2]]
    depth = int(header2)
    assert depth <= 65535
...
def experiment(args):
    ...
    print("Load data")
    allFace = []
    for curF in glob.glob('data/face/*.pgm'):
        file = open(curF, 'rb')
        im = np.array(read_pgm(file))
        file.close()
        allFace.append(im.flatten())
    allFace = np.array(allFace)
    X = allFace.astype(np.float32)
    X = X/np.max(X)
